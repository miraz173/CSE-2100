...
let slot='', spaces='&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp||&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp'; 
let x=1, y=1;
...
function displayResult2(response) {
    response.result.values.forEach((row, index) => {
        ...
        // {
            ...
            {
                console.log(nowtime);console.log(eventtime);
                if(nowtime <= eventtime)
                {
                    ...
                    slot+="<div class='event_item' style='width: 90vw; background: linear-gradient(90deg, #84dcff77, #201f5579);'>"
                    +"<div class='ei_Dot dot_active'></div><div id='ei_Title' style='width: 8vw;'>"
                    +startingTime.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })
                    +"</div><div style='width: 16vw;'>"+title+"</div><div style='width: 44vw;'><i>"+description+"</i></div></div>"
                }
            }
            else if(eventdate > nowdate)
            {
                ...
                slot+="<div class='event_item' style='width: 90vw; background: linear-gradient(90deg, #84dcff77, #201f5579);'>"
                +"<div class='ei_Dot'></div><div id='ei_Title' style='width: 8vw;'>"
                +startingTime.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })
                +"</div><div style='width: 16vw;'>"+title+"</div><div style='width: 44vw;'><i>"+description+"</i></div></div>"
            }
