...
function connectionStatus() {
    fetch('https://static-global-s-msn-com.akamaized.net/hp-neu/sc/2b/a5ea21.ico?d=' + Date.now())
        ...
        .catch(error => {
            console.log("No Internet connection", error);// The resource could not be reached
            let hr = document.getElementsByTagName("hr");
...
function openTab(evt, cityName) {
    ...
    for (i = 0; i < tabcontent.length; i++) {
        ...
        // }
        // else{
        // 	document.getElementById("main").style.background="#201F55"//"#3c3c7f"; //#194a7b
        // }
    }
...
$(function () {
    ...
    setInterval(function () {
        ...
        let nextTab = onTab.index() < tabs.length - 2 ? onTab.next() : tabs.first();
        if (onTab.index() == 2) {
            scrollTime = 12000;
