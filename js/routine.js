...
// localStorage.routineLoaded=false; //gloab variable
let tr = [], routineLoaded=0;
                ...
                row.forEach((val, ind) => {
                    if (row[ind] !== row[ind - 1]) {
                        if (row[ind] === row[ind + 1] && row[ind] === row[ind + 2]) {
                            tableBody += "<td colspan='3'>" + val + "</td>";
                            td.push(3);
                        } else if (row[ind] === row[ind + 1] && row[ind] !== row[ind + 2]) {
                            tableBody += "<td colspan='2'>" + val + "</td>";
                            td.push(2);
                        } else {
                            tableBody += "<td>" + val + "</td>";
                            td.push(1);
...
function loadData(spreadsheetId) {
    ...
    if(timeCheck>=17){//17===5:00 pm, after class ends, show routine of next
        if(day===6){day=0;}
        else{ day++;}
    }
...
document.addEventListener("gapi-loaded", (e) => {
    loadData(spreadsheetId); //range not passed as it will be dynamically calculated/specified
});
...
function highlightCol() {
    ...
    if (routineLoaded==0) {
        window.location.reload();       //reload if not web already loaded
    }
}

setInterval(highlightCol, 5000);//5 second