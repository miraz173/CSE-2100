...
let slot='', title='', description='', startingTime='', endingTime='', location='', room='', poster='', eventsLoaded='';
// localStorage.eventsLoaded=false;
...
function displayResult2(response) {
    response.result.values.forEach((row, index) => {
        if (index === 1)
        {
            ...
            document.getElementById('bigPic').style="background-image: linear-gradient(to bottom right, rgba(21, 0, 255, 0.34), rgba(255, 0, 238, 0.26)), url(" +poster+ ");";
        ...
        else if(index > 1 && index <= 5)
        {
            ...
            slot+=
            "<div class='smallerBigPic' style='background-image: linear-gradient(to bottom right, rgba(255, 0, 128, 0.577), rgba(0, 204, 255, 0.49)),url("
            +poster+");'><div class='smallerLeftBoxText'><h2>"+title+"</h2><h5>"+description+"</h5></div></div>";
        }
    });
